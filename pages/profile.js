    ...
    const fetchUserData = async () => {
      try {
        if (user) {
          let response;
          if (user.id === 3) {
            // Fetch all users data otherwise
            response = await fetch('/api/auth/users');
          } else {
            // Fetch single user data if user ID is not 3
            response = await fetch(`/api/auth/oneuser?id=${user.id}`);
          }
        ...
        {users ? (
          <div className="table-responsive">
            <table className="table table-dark table-hover">
              {(user.id === 3) && <caption>List of users</caption>}
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Name</th>
